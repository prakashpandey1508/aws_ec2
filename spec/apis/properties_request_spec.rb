...
RSpec.describe "GET /api/v1/properties", type: :request do
  ...
  describe "DELETE /api/v1/properties/:id" do
   ...
   #custome api Rspec in Property for count appointment and Comments

     describe "Custom api for the appointmments" do
      let(:company){create(:company)}
      let(:agent) { create(:agent,company:company) }
      let(:propertytype){create(:propertytype)}
      let(:property){create(:property, agent:agent, propertytype:propertytype)}
      ...
      let(:propertytype){create(:propertytype)}
      let(:property){create(:property, agent:agent, propertytype:propertytype)}
      context "if appointments exists" do
        before { 
          get "/api/v1/properties/property_appointments", params: { id: property.id } 

          
        }
        it 'returns the number of likes on the proeprty' do
          expect(JSON.parse(response.body)) == property.appointments.count
        end
      end
